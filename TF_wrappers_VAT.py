...
def LSM(inputs):
    ...
    input=inputs-tf.reduce_max(inputs,3, keep_dims=True) # I assume this step has something to do with numerical stabiity?
...
def VAT_loss(X_total, logits_total, input_depth, num_classes, epsilon, size4net1, size4net2, is_training_flag=False): # Original VAT code at: https://github.com/takerum/vat_tf/blob/master/vat.py
    ...
    # Questions: what is x, what's up with the normalization method? --> Review method for obtaining r_adv!!!!
	# Why is the gradient propogated only through the adverserially perturbed stream?
